  ...
  Future<void> _editCoordinates(Restaurant r) async {
    ...
    if (lat == null || lng == null) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('올바른 숫자를 입력하세요.')));
      return;
    }
    ...
    if (success) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('좌표가 저장됐어요. 이제 지도에 표시됩니다.')));
      _load();
    } else {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('좌표 저장에 실패했습니다.')));
    }
  }
